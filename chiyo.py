...
async def on_message(message):
	...
	if 'https://akatsuki.pw/b/' in message.content:
		...
		fyusagfa = f.replace('-taiko','')
		g = f.replace('?mode=3','')
		t = requests.get(f'https://akatsuki.pw/api/get_beatmaps?limit=1&b={g}&m=0')
		if not t:
			return
		title = t.json()[0]['title']
		id_b = t.json()[0]['beatmap_id']
	...
	if 'https://akatsuki.pw/d/' in message.content:
		...
		f = e.replace('?mode=2','')
		g = f.replace('?mode=3','')
		t = requests.get(f'https://akatsuki.pw/api/get_beatmaps?limit=1&s={g}&m=0')
		if not t:
			return
		title = t.json()[0]['title']
		id_b = t.json()[0]['beatmap_id']
	...
	if 'https://akatsuki.pw/d/' in message.content and '-taiko' in message.content:
		...
		t = requests.get(f'https://akatsuki.pw/api/get_beatmaps?limit=1&s={g}&m=1')
		if not t:
			return
		title = t.json()[0]['title']
		id_b = t.json()[0]['beatmap_id']

...
class test(commands.Cog):
	...
	async def reg(self, ctx, *, arg):
	  ...
	  if not getprofile:
	    ...
	    rank = getprofile.json()['stats'][0]['std']['global_leaderboard_rank']
	    countryrank = getprofile.json()['stats'][0]['std']['country_leaderboard_rank']
	    pp = getprofile.json()['stats'][0]['std']['pp']
	    acc = getprofile.json()['stats'][0]['std']['accuracy']
	    owo = round(acc, 2)
	    playcount = getprofile.json()['stats'][0]['std']['playcount']
	  ...
	  embed=discord.Embed()
	  embed=discord.Embed(title=f"Regular Stats for {name}", url=f"https://akatsuki.pw/rx/u/{userid}?mode=0", color=0xb6ebf1)

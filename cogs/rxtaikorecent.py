...
class test(commands.Cog):
	...
	@commands.command()
	async def rxtaikorecent(self, ctx, *, arg):
	  ...
	  rrecent = requests.get(f"https://akatsuki.pw/api/v1/users/scores/recent?rx=1&name={arg}&mode=1")
	  if not rrecent:
	  ...
	  beatmaprecent = requests.get(f"https://akatsuki.pw/api/get_beatmaps?limit=1&m=1&b={rcmapid}")
	  ...
	  embed=discord.Embed()
	  embed=discord.Embed(title=f"{beatmapnamerc} by {beatmapartrc}", url=f"https://akatsuki.pw/b/{rcmapid}", color=0xb6ebf1)

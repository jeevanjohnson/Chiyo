...
class test(commands.Cog):
	...
	async def taiko(self, ctx, *, arg):
	  ...
	  if not getprofile:
	    ...
	    rank = getprofile.json()['stats'][0]['taiko']['global_leaderboard_rank']
	    countryrank = getprofile.json()['stats'][0]['taiko']['country_leaderboard_rank']
	    pp = getprofile.json()['stats'][0]['taiko']['pp']
	    acc = getprofile.json()['stats'][0]['taiko']['accuracy']
	    owo = round(acc, 2)
	    playcount = getprofile.json()['stats'][0]['taiko']['playcount']
	  ...
	  embed=discord.Embed()
	  embed=discord.Embed(title=f"Taiko Stats for {name}", url=f"https://akatsuki.pw/rx/u/{userid}?mode=0", color=0xb6ebf1)
